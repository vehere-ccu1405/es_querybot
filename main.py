...
from field_enrichment import FIELD_ENRICHMENT
...
class QueryAnalyzer:
    def __init__(self,embed_model_path=EMBED_MODEL,embedding_dim=EMBED_DIMENSION):
    ...
    def build_index(self,mapping_json_path=MAPPING_JSON_PATH,index_path=INDEX_PATH,data_path=DATA_PATH):
        ...
        print(f"[build index] Embedding {len(self.docs)} fields...")
        ...
        print(f"[build_index] Done. {len(self.docs)} fields indexed.")
    ...
    def load_index(self, index_path=INDEX_PATH, data_path=DATA_PATH):
        ...
        print(f"[load_index] Loaded {len(self.docs)} fields.") 
    
    
    def query(self, nl_input:str, top_k:int=5):
...
if __name__ == "__main__":
    ...
    query_analyzer.build_index()
    query_analyzer.load_index()
    
    queries = [
        "who sent this email?",
        "what country is the source IP from?",
        "TLS fingerprint of the client",
        "how long was the session?",
        "which file was transferred over SMB?",
    ...
    for q in queries:
        print(f"\nQuery: '{q}'")
        print("-" * 60)
        for r in query_analyzer.query(q, top_k=3):
            print(r)
